      ...
      do
        ...
        do
          echo -e "THREADS\tTHROUGHPUT\tHTM_COMMITS\tSGL_COMMITS\tHTM_ABORTS\tHTM_CONFLICT\tHTM_CAPACITY\tHTM_EXPLICIT\tHTM_OTHER" \
            > $DATA/VACATION_${sol}-${file_name[$log_sol]}-LOGS${t}_TXS${NB_TXS}_s${s}.tsv
          ...
          do
            ...
            echo "LOG_REPLAY_PARALLEL NB_REPLAYERS=${NB_REPLAYERS} PINNING=${PINNING} FLUSH_LAT=0 ${sol} ${log_sol} \
              ERROR_FILE=$DATA/error_VACATION_${sol}-${file_name[$log_sol]}-LOGS${t}_TXS${NB_TXS}_s${s}.tsv \
              PROFILE_FILE=$DATA/prof_VACATION_${sol}-${file_name[$log_sol]}-LOGS${t}_TXS${NB_TXS}_s${s}.tsv \
              LOG_REPLAY_STATS_FILE=$DATA/log_VACATION_${sol}-${file_name[$log_sol]}-LOGS${t}_TXS${NB_TXS}_s${s}.tsv " \
              > nvhtm_params.txt
            ./vacation/vacation -n2 -q90 -u98 -r1048576 -t $NB_TXS -c $t \
              | sed '/^[0-9]*\t[0-9]*.[0-9]*\t[0-9]*\t[0-9]*\t[0-9]*\t[0-9]*\t[0-9]*\t[0-9]*\t[0-9]*$/p' -n \
              | tail -n 1 >> $DATA/VACATION_${sol}-${file_name[$log_sol]}-LOGS${t}_TXS${NB_TXS}_s${s}.tsv
            sleep 0.05s
          done
        done
      done
