...
struct bplus_non_leaf *non_leaf_new()
{
	...
	(struct bplus_non_leaf*)TM_MALLOC(sizeof(struct bplus_non_leaf));
#endif
...
struct bplus_leaf *leaf_new()
{
	...
	(struct bplus_leaf*)TM_MALLOC(sizeof(struct bplus_leaf));
#endif
...
{
        long i;
        struct bplus_node *node = TM_SHARED_READ_P(tree->root);
        ...
        while (node != nullptr) {
                ...
                case BPLUS_TREE_NON_LEAF:
                        ...
                        if (i >= 0) {
                                node = TM_SHARED_READ_P(nln->sub_ptr[i + 1]);
                        } else {
                                i = -i - 1;
                                node = TM_SHARED_READ_P(nln->sub_ptr[i]);
                        }
                        break;
...
{       
        ...
        if (node->children == TM_SHARED_READ(tree->order)) {
                ...
                sibling->next = TM_SHARED_READ_P(node->next);
                ...
                if (insert < split) {
                        ...
                        sibling->sub_ptr[0] = TM_SHARED_READ_P(node->sub_ptr[split]);
                        ...
                        for (i = split, j = 0; i < TM_SHARED_READ(tree->order) - 1; i++, j++) {
                                ...
                                sibling->sub_ptr[j + 1] = TM_SHARED_READ_P(node->sub_ptr[i + 1]);
                                TM_SHARED_WRITE_P(TM_SHARED_READ_P(node->sub_ptr[i + 1])->parent, sibling);
                        }
                        ...
                        for (i = TM_SHARED_READ(node->children) - 2; i > insert; i--) {
                                ...
                                TM_SHARED_WRITE_P(node->sub_ptr[i + 1], TM_SHARED_READ_P(node->sub_ptr[i]));
                        }
                        ...
                        /* sibling node's first sub-node */
                        sibling->sub_ptr[0] = TM_SHARED_READ_P(node->sub_ptr[split + 1]);
        ...
        if (split) {
                if (TM_SHARED_READ_P(tree->head[level + 1]) == nullptr) {
                        ...
                        /* Trace upwards */
                        sibling->parent = TM_SHARED_READ_P(node->parent);
...
long leaf_insert(struct bplus_tree *tree, struct bplus_leaf *leaf, long key, long data)
{
        ...
        if (leaf->entries == TM_SHARED_READ(tree->entries)) {
                ...
                /* splited sibling node */
                sibling = leaf_new();
                sibling->next = TM_SHARED_READ_P(leaf->next);
        ...
        if (split) {
                struct bplus_non_leaf *parent = TM_SHARED_READ_P(leaf->parent);
                if (parent == nullptr) {
                        /* new parent */
                        parent = non_leaf_new();
                ...
                if (parent != nullptr) {
                        ...
                        if (borrow == BORROW_FROM_LEFT) {
                                if (TM_SHARED_READ(sibling->entries) > (TM_SHARED_READ(tree->entries) + 1) / 2) {
                                        ...
                                        /* delete merged leaf */
                                        TM_SHARED_WRITE_P(sibling->next, TM_SHARED_READ_P(leaf->next));
                                        leaf_delete(leaf);
                                        /* trace upwards */
                                ...
                                if (TM_SHARED_READ(sibling->entries) > (TM_SHARED_READ(tree->entries) + 1) / 2) {
                                        ...
                                        TM_SHARED_WRITE_P(leaf->next, TM_SHARED_READ_P(sibling->next));
                                        leaf_delete(sibling);
                                        /* trace upwards */
