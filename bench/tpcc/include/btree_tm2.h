...
public:
  ...
  __attribute__((transaction_safe)) void insert(TM_ARGDECL KEY key, VALUE value) {
    //GCC warns that this may be used uninitialized, even though that is untrue.
    InsertionResult result = {KEY(), 0, 0};
    bool was_split;
    unsigned long d = SHARED_READ(depth);
    if (d == 0) {
      //The root is a leaf node
      void *temp_node = SHARED_READ_P(root);
      was_split = leaf_insert(TM_ARG reinterpret_cast < LeafNode * >(temp_node), key, value, &result);
    } else {
      //The root is an inner node
      void *temp_node = SHARED_READ_P(root);
      was_split = inner_insert(TM_ARG reinterpret_cast < InnerNode * >(temp_node), d, key, value, &result);
    }
    if (was_split) {
      ...
      SHARED_WRITE(depth, ++d);
      SHARED_WRITE_P(root, rootProxy);
    }
  }
  ...
  __attribute__((transaction_safe)) bool find(TM_ARGDECL const KEY & key, VALUE * value = 0)const {
    const InnerNode * inner;
    register const void *node = SHARED_READ_P(root);
    register unsigned long d = SHARED_READ(depth);
    register unsigned index;
    while (d-- != 0) {
      inner = reinterpret_cast < const InnerNode *>(node);
      unsigned long num_keys_1 = SHARED_READ(inner->num_keys);
      index = inner_position_for(TM_ARG key, inner->keys, num_keys_1);
      node = SHARED_READ_P(inner->children[index]);
    }
    const LeafNode *leaf = reinterpret_cast < const LeafNode * >(node);
    unsigned long num_keys = SHARED_READ(leaf->num_keys);

    index = leaf_position_for(TM_ARG key, leaf->keys, num_keys);
    if (index < num_keys) {
      intptr_t temp_key = SHARED_READ(leaf->keys[index]);
      if (temp_key == key) {
        VALUE temp_value = (VALUE) SHARED_READ_P(leaf->values[index]);
  ...
  __attribute__((transaction_safe)) bool del(TM_ARGDECL const KEY & key) {
    InnerNode *inner;
    register void *node = SHARED_READ_P(root);
    register unsigned long d = SHARED_READ(depth);
    register unsigned index;
    while    (d-- != 0) {
    ...
    if (temp_key == key) {
      SHARED_WRITE_P(leaf->values[index], 0);
      return true;
    } else {
      return false;
    }
  ...
  __attribute__((transaction_safe)) bool findLastLessThan(TM_ARGDECL const KEY & key, VALUE * value = 0, KEY * out_key = 0)const {
    const void *node = SHARED_READ_P(root);
    unsigned long d = SHARED_READ(depth);
    while (d-- != 0) {
      const InnerNode *inner = reinterpret_cast < const InnerNode * >(node);
      unsigned long num_keys_1 = SHARED_READ(inner->num_keys);
      unsigned int pos = inner_position_for(TM_ARG key, inner->keys, num_keys_1);
      //We need to rewind in the case where they are equal
      if  (pos > 0) {
        intptr_t temp_key = SHARED_READ(inner->keys[pos - 1]);
        if (key == temp_key)
        pos -= 1;
      }
      node = SHARED_READ_P(inner->children[pos]);
    }
    const LeafNode *leaf = reinterpret_cast < const LeafNode * >(node);
    unsigned long num_keys = SHARED_READ(leaf->num_keys);
    unsigned int pos = leaf_position_for(TM_ARG key, leaf->keys, num_keys);

    if (pos <= num_keys) {
      pos -= 1;
      if (pos < num_keys) {
        intptr_t temp_key_2 = SHARED_READ(leaf->keys[pos]);

        if (key == temp_key_2)
        pos -= 1;
      }
      if (pos < num_keys) {
        VALUE temp_value = (VALUE) SHARED_READ_P(leaf->values[pos]);
        ...
        if (temp_value) {
          ...
          if (out_key != NULL) {
            intptr_t temp_key_3 = SHARED_READ(leaf->keys[pos]);
            *out_key = temp_key_3;
          }
          return true;
        } else {
          ...
          intptr_t temp_key_4 = SHARED_READ(leaf->keys[pos]);
          return findLastLessThan(TM_ARG temp_key_4, value, out_key);
        }
      }
    }
    return false;
...
private:
  ...
  enum NodeType {
    ...
    static unsigned leaf_position_for(
      ...
      unsigned k = 0;
      intptr_t temp_key = SHARED_READ(keys[k]);
      while    ((k < num_keys) && (temp_key < key)) {
        ++k;
        temp_key = SHARED_READ(keys[k]);
      }
      return k;

    //Returns the position where 'key' should be inserted in an inner node
    // that has the given keys.
    static inline unsigned inner_position_for(
      TM_ARGDECL const KEY &key,
      const KEY            *keys,
      unsigned long         num_keys
    ){
    ...
    __attribute__((transaction_safe)) bool leaf_insert(
      ...
      unsigned long num_keys = SHARED_READ(node->num_keys);
      // assert(num_keys <= M);
      ...
      if (num_keys >= M) {
        ...
        for (unsigned j = 0; j < new_sibling->num_keys; ++j) {
          intptr_t temp_key = SHARED_READ(node->keys[treshold + j]);
          new_sibling->keys[j] = temp_key;
          VALUE temp_value = (VALUE) SHARED_READ_P(node->values[treshold + j]);
          new_sibling->values[j] = temp_value;
        }
        SHARED_WRITE(node->num_keys, treshold);
        ...
        was_split = true;
        result->key = SHARED_READ(new_sibling->keys[0]);
        result->left = node;
    ...
    __attribute__((transaction_safe)) static void leaf_insert_nonfull(
      TM_ARGDECL LeafNode *node,
      KEY                 &key,
      VALUE               &value,
      unsigned             index
    ){
      intptr_t temp_key = SHARED_READ(node->keys[index]);
      if (temp_key == key) {
        //We are inserting a duplicate value.
        // Simply overwrite the old one
        VALUE temp_value = (VALUE) SHARED_READ_P(value);
        SHARED_WRITE_P(node->values[index], temp_value);
      } else {
        unsigned long num_keys = SHARED_READ(node->num_keys);

        //The key we are inserting is unique
        for (unsigned i = num_keys; i > index; --i) {
          intptr_t temp_key_2 = SHARED_READ(node->keys[i - 1]);

          SHARED_WRITE(node->keys[i], temp_key_2);
          VALUE temp_value_2 = (VALUE) SHARED_READ_P(node->values[i - 1]);

          SHARED_WRITE_P(node->values[i], temp_value_2);
        }
        SHARED_WRITE(node->num_keys, ++num_keys);
        //node->num_keys++;
        SHARED_WRITE(node->keys[index], key);
        VALUE temp_value_3 = (VALUE) SHARED_READ_P(value);

        SHARED_WRITE_P(node->values[index], temp_value_3);
      }
    }

    __attribute__((transaction_safe)) bool inner_insert(
      TM_ARGDECL InnerNode *node,
      ...
      if (num_keys >= N) {
        ...
        for (unsigned i = 0; i < new_sibling->num_keys; ++i) {
          intptr_t temp_key = SHARED_READ(node->keys[treshold + i]);
          new_sibling->keys[i] = temp_key;
          new_sibling->children[i] = SHARED_READ_P(node->children[treshold + i]);
        }
        new_sibling->children[new_sibling->num_keys] = SHARED_READ_P(node->children[num_keys]);

        SHARED_WRITE(node->num_keys, treshold - 1);
        //Set up the return variable
        was_split = true;
        intptr_t temp_key_2 = SHARED_READ(node->keys[treshold - 1]);

        result->key = temp_key_2;
        result->left = SHARED_READ_P(node);
        result->right = new_sibling;
        //Now insert in the appropriate sibling
        if (key < result->key) {
          inner_insert_nonfull(TM_ARG node, current_depth, key, value);
        } else {
          inner_insert_nonfull(TM_ARG new_sibling, current_depth, key, value);
        }
      } else {
        //No split
        inner_insert_nonfull(TM_ARG node, current_depth, key, value);
    ...
    __attribute__((transaction_safe)) void inner_insert_nonfull(
      TM_ARGDECL InnerNode *node,
      unsigned long         current_depth,
      KEY                  &key,
      VALUE                &value
      ...
      //Simple linear search
      unsigned long num_keys = SHARED_READ(node->num_keys);
      unsigned index = inner_position_for(TM_ARG key, node->keys, num_keys);
      //GCC warns that this may be used uninitialized, even though that is untrue.
      InsertionResult result = {
        KEY(), 0, 0
      };
      ...
      if (current_depth - 1 == 0) {
        ...
        void *temp_node = SHARED_READ_P(node->children[index]);

        cout << "child node " << temp_node << "\n";

        if (temp_node == NULL) {
          temp_node = new_leaf_node();
          //TODO:MEMORY LEAK ! !!
          SHARED_WRITE_P(node->children[index], temp_node);
          cout << "  -- was NULL, alloced " << temp_node << "\n";
        }
        was_split = leaf_insert(TM_ARG reinterpret_cast < LeafNode * >(temp_node), key, value, &result);
      } else {
      ...
      if (was_split) {
        if (index == num_keys) {
          ...
          SHARED_WRITE(node->keys[index], result.key);
          SHARED_WRITE_P(node->children[index], result.left);
          SHARED_WRITE_P(node->children[index + 1], result.right);
          SHARED_WRITE(node->num_keys, ++num_keys);
          cout << " ++ was_split, node->children[" << index << "] = " << node->children[index]
            << " node->children[" << index+1 << "] = " << node->children[index+1] << "\n";
        } else {
          //Insertion not at the rightmost key
          void *temp_node_2 = SHARED_READ_P(node->children[num_keys]);

          SHARED_WRITE_P(node->children[num_keys + 1], temp_node_2);
          cout << " -- was_split, node->children[" << num_keys + 1 << "] = " << node->children[num_keys + 1] << "\n";
          for (unsigned i = num_keys; i != index; --i) {
            void *temp_node_3 = SHARED_READ_P(node->children[i - 1]);

            SHARED_WRITE_P(node->children[i], temp_node_3);
            intptr_t temp_key_2 = SHARED_READ(node->keys[i - 1]);

            SHARED_WRITE(node->keys[i], temp_key_2);
            cout << " -- was_split, node->children[" << i + 1 << "] = " << node->children[i] << "\n";
          }
          SHARED_WRITE_P(node->children[index], result.left);
          SHARED_WRITE_P(node->children[index + 1], result.right);
          SHARED_WRITE(node->keys[index], result.key);
