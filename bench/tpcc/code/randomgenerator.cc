...
static thread_local unsigned long RND_FN_seed = 1234;
...
RealRandomGenerator::RealRandomGenerator() {
    ...
    RND_FN_seed = (unsigned long)time(NULL);
}
...
//     return rand_int;
    ...
    unsigned long rand_int = RAND_R_FNC(RND_FN_seed);
    // unsigned int range = upper - lower + 1;

    // if (range == 0) return lower;

    // unsigned int totalBits = sizeof(unsigned int) << 3;
    // unsigned int positionFirstBitTo1 = totalBits - __builtin_clz(range);
    // unsigned long mask_rand_int = ((unsigned)-1) >> positionFirstBitTo1;
    // mask_rand_int <<= positionFirstBitTo1;
    // unsigned int mask_rand_int2 = rand_int & (~mask_rand_int);

    // int final_rand = mask_rand_int2 + lower;
    // // while (final_rand < lower) final_rand++;
    // while (final_rand > upper) final_rand -= range;

    // return final_rand;
    return (int)((rand_int % (upper - lower + 1)) + lower);
}
